...
{
    public class VATService : DbService
    {
        public VATService(IDbHelper db) : base(db)
        {
        }

        public IEnumerable<VATDisplayView> GetViewVAT()
        {
            var result = Db.VATDisplayViewRepository.FindAll();
            return result;
        }
        ...
        public DataSourceResult GetDataSourceResult(DataSourceRequest request)
        {
            ...
            var result = genericDataTableQuery.GetData<VATDisplayView>(query);
            return result;
        }
        ...
        public string CreateBusinessCategory(IEnumerable<VATDisplayView> busctg)
        {
            ...
            using (DbHelper db = new DbHelper(true))
            {
                try
                {
                    foreach (VATDisplayView view in busctg)
                    {
