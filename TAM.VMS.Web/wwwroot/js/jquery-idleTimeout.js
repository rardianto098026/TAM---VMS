...
(function ($) {
    $.fn.idleTimeout = function (userRuntimeConfig) {
        ...
        var defaultConfig = {
            ...
            // idle settings
            idleTimeLimit: 10800,         // 'No activity' time limit in seconds. 1200 = 20 Minutes. 10800 = 180 minutes equeal web config settings
            ...
            modalDialogID: 'idle-timeout-modal',
            ...
            openWarningDialog, openWarningModalCustom, dialogTimer, checkDialogTimeout, startDialogTimer, stopDialogTimer, isDialogOpen, isModalOpen, destroyWarningDialog, countdownDisplay, // warning dialog
            logoutUser;
        ...
        activityDetector = function () {
            ...
            $('body').on(currentConfig.activityEvents, function () {
                ...
                //}
                if (!currentConfig.enableDialog || (currentConfig.enableDialog && isModalOpen() !== true)) {
                    startIdleTimer();
                    //$('#activity').effect('shake'); // added for demonstration page
                }
            });
        ...
        checkIdleTimeout = function () {
            ...
            if ($.now() > timeIdleTimeout) {
                ...
                } else if (currentConfig.enableDialog && isModalOpen() !== true) {
                    openWarningModalCustom(); // start timing the warning dialog  
                    startDialogTimer();
                }
        ...
        //----------- WARNING MODAL CUSTOM FUNCTIONS --------------//
        openWarningModalCustom = function () {
            $("#idle-timeout-modal-logout").on('click', function (event) {
                logoutUser();
            });

            $("#idle-timeout-modal-keepalive").on('click', function (event) {
                destroyWarningModal();
                stopDialogTimer();
                startIdleTimer();
            });

            let $modaltimeout = $('#' + currentConfig.modalDialogID);
            $modaltimeout.modal({
                show: true,
                keyboard: false,
                backdrop: 'static'
        ...
        isModalOpen = function () {
            let dialogOpen = $('#' + currentConfig.modalDialogID).is(":visible");
